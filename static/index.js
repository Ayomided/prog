...
(function oneko() {
	...
	const nekoSites = ["adediiji.uk", "localhost"];
	...
	try {
		...
		if (tmp && tmp[1]) nekoPosX = Number.parseInt(tmp[1]);
		tmp = searchParams.find((a) => a[0] === "caty");
		if (tmp && tmp[1]) nekoPosY = Number.parseInt(tmp[1]);
		tmp = searchParams.find((a) => a[0] === "catdx");
		if (tmp && tmp[1]) mousePosX = Number.parseInt(tmp[1]);
		tmp = searchParams.find((a) => a[0] === "catdy");
		if (tmp && tmp[1]) mousePosY = Number.parseInt(tmp[1]);
	} catch (e) {
