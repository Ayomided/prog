...
const queryArticlesBySlug = `-- name: QueryArticlesBySlug :one
select id, created_at, updated_at, title, slug, filename from articles where slug=?
`

func (q *Queries) QueryArticlesBySlug(ctx context.Context, slug string) (Article, error) {
	row := q.db.QueryRowContext(ctx, queryArticlesBySlug, slug)
	var i Article
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
